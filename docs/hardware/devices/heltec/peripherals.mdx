...
IF the GPS module does not turn on at all when using a N2222A transistor can be caused by the  inconsistency of available modules of the GT-U7, needing more or less power to run. The 0.6 volt  voltage drop on the transistor results in a voltage of 2.7 volts coming from the 3.3volt rail on the Heltec V3 board,  which is way under the GT-U7 rated 3.6-  5volts  operating voltage. You can be lucky or unlucky.  
